...
/**
 * HKDF-SHA-256 générique : 256 bits tirés d'un matériau quelconque.
 *
 * Elle est ici, et pas chez ses appelants, pour la même raison que le reste du
 * fichier : il ne doit exister qu'une dérivation dans ce dépôt. `deriveWriteKey`
 * en est le cas particulier historique ; l'appairage (ADR-003) en est un second,
 * qui part d'un secret ECDH au lieu d'une clé de session.
 *
 * @param {{ikm:Uint8Array, salt?:Uint8Array, info:Uint8Array|string}} args
 * @returns {Promise<Uint8Array>} 32 octets
 */
export async function hkdf256({ ikm, salt = new Uint8Array(0), info }) {
  const materiel = await subtle().importKey('raw', asBytes(ikm), 'HKDF', false, ['deriveBits']);
  const bits = await subtle().deriveBits(
    { name: 'HKDF', hash: 'SHA-256', salt: asBytes(salt), info: typeof info === 'string' ? utf8(info) : asBytes(info) },
    materiel,
    KEY_LEN * 8,
  );
...
export async function deriveWriteKey(key) {
  return hkdf256({ ikm: assertKey(key, 'clé de session'), info: HKDF_INFO_WRITE });
}
