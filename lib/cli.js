...
export async function run(argv, io = {}) {
  ...
  const ctx = { stdout, stderr, home: { home }, now, fetchImpl, sleep, readFile, uiBase, signal };
...
async function cmdJoin(values, positionals, ctx, log) {
  ...
  if (!roster) {
    ...
    log(connue
      ...
      : 'aucun roster trouvé sur le bus : la durée de vie ne peut être confirmée (cache ntfy expiré ?)');
  }
...
async function cmdExport(values, positionals, ctx, log) {
  const url = requireUrl(positionals);
  const { topic, server } = parseSessionUrl(url);
...
function cleDeRelecture({ archive, values, ctx }) {
  if (values.url) {
    const { topic, key } = parseSessionUrl(values.url);
...
async function cmdMigrate(values, positionals, ctx, log) {
  const url = requireUrl(positionals);
  const { topic, key, ro, server, uiBase } = parseSessionUrl(url);
  const connue = tryLoad(topic, ctx);
  const participant = exigerDroitDEcriture({ ro, connue, values, ctx });
  ...
  ctx.stdout(buildSessionUrl({ topic: nouveau, key, server, uiBase: values.ui ?? uiBase ?? ctx.uiBase }));
  return EXIT.OK;
}
...
export { USAGE, NetworkError, LimitError, UsageError, IntegrityError, ArchiveError };
