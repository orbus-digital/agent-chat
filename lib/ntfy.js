 ...
 *   HTTPS — aucune requête en clair ne quitte la machine (voir `serveur.js`).
 *
 * Cette dernière garde est ici, et pas seulement chez l'appelant, parce que
 * c'est ici qu'est le `fetch` : une couche qui parle au réseau ne peut pas
 * déléguer à ses appelants le soin de vérifier ce qu'elle va émettre.
 */

import { byteLength } from './bytes.js';
import { assertBaseTransport } from './serveur.js';
...
export async function publish({
  ...
  const racine = assertBaseTransport(base);
  ...
  for (let essai = 0; essai < attempts; essai++) {
    let res;
    try {
      res = await fetchImpl(`${racine}/${topic}`, { method: 'POST', headers, body, signal });
    } catch (cause) {
...
export async function poll({ base, topic, since = 'all', fetchImpl = fetch, signal }) {
  const url = `${assertBaseTransport(base)}/${topic}/json?poll=1&since=${encodeURIComponent(since)}`;
  let res;
  try {
...
export function subscribe({
  ...
  // Refus immédiat et synchrone : passer par `onError` ferait entrer une base
  // refusée dans la boucle de reconnexion, où elle serait réessayée toutes les
  // secondes sans que personne ne sache pourquoi.
...
async function streamSse({ base, topic, since, fetchImpl, signal, onMessage }) {
  const url = `${assertBaseTransport(base)}/${topic}/sse?since=${encodeURIComponent(since)}`;
